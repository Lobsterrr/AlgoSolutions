 ...
 *  check. Since each version is developed based on 
 *  the previous version, all the versions after a bad 
 *  version are also bad.

 * Suppose you have n versions [1, 2, ..., n] and you 
 * want to find out the first bad one, which causes 
 * all the following ones to be bad.

 * You are given an API bool isBadVersion(version) 
 * which will return whether version is bad. Implement 
